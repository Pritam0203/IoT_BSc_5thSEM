...
menubar=Menu(root)
def search():
     os.system("python /home/rudra/code/CMS/src/search.py")
def about():
...
def web1():
     webview.create_window('FBI TOP 10 WANTED LIST','https://www.fbi.gov/wanted/topten')
     webview.start()
def exit11():
     root.quit()
def openNewWindow():
    ...
    my_list = ["Share a secret code word","You and your parents should agree on a code","that is easy for you to remember to use in emergency", "Stay away from strangers", "Grownups should NOT ask kids to do things that other adults can do for them.", "If someone follows you on foot or in a car, STAY AWAY.","If you think you are not secure","Call us immediately +91 8018295791(AGENT RUDRA)","\n","\n","\n","SATYAMEV JAYATE"]
    txt_output = Text(newWindow, height=25, width=60)
    txt_output.pack(pady=30)
    ww=Label(newWindow,text="                   RUDRA'S CRIME MANAGEMENT SYSTEM COPYRIGHT 2023",height=1,bg='white',fg='black').place(x=40,y=550)
    for item in my_list:
         txt_output.insert(END, item + "\n")
         pass
def crime():
     os.system("python /home/rudra/code/CMS/src/demo1.py")
menubar.add_command(label="CRIME LIST",command=crime)
menubar.add_command(label="TIPS FROM US", command=openNewWindow)
menubar.add_command(label="FBI TOP 10 LIST",command=web1)
menubar.add_command(label="ABOUT SOFTWARE",command=about)
menubar.add_command(label="SEARCH FOR A CRIMINAL",command=search)
menubar.add_command(label="EXIT",command=exit11)
root.config(menu=menubar)
canvas_widget=Canvas(root,width=1200,height=800)
canvas_widget.pack()
img=ImageTk.PhotoImage(Image.open("/home/rudra/code/CMS/src/16.jpg"))
